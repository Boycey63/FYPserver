...
var file = './src/public/data/trainingData.json';
 ...
 */
var saveTrainingReviews = function saveTrainingReviews() {
    //Open big review file
    var instream = fs.createReadStream('./src/public/data/review.json');
    ...
    var numberOfReviews = 5261670;

    var i = 0;

    var reviewData = fs.openSync('./src/public/data/reviewdata.json', 'a');
    ...
    rl.on('line', function (line) {
        var reviewString = JSON.parse(line);
        //438160 5261670
        var numberofReviewsTrained = 350000;
        if (!ended) {
            if (i < numberOfReviews) {
                ...
                if (one === numberofReviewsTrained && two === numberofReviewsTrained && three === numberofReviewsTrained && four === numberofReviewsTrained && five === numberofReviewsTrained) {
                    i = numberOfReviews + 1;
                    nbc.consolidate();
                    var variable = nbc.exportJSON();
                    var exportData = JSON.stringify(variable);
...
var saveTrainingReviewsUseful = function saveTrainingReviewsUseful() {
    //Open big review file
    var instream = fs.createReadStream('./src/public/data/review.json');
    ...
    // Reads big file and learns based off it ... need to create new document...
    var numberOfReviews = 5261668;

    var i = 0;
    var useful = 4;

    var reviewData = fs.openSync('./src/public/data/reviewdatauseful2.json', 'a');
    ...
    rl.on('line', function (line) {
        var reviewString = JSON.parse(line);
        //438160 5261670
        var numberofReviewsTrained = 55000;
 ...
 */
var saveTestReviews = function saveTestReviews() {
    //Open big review file
    var instream = fs.createReadStream('./src/public/data/review.json');
    ...
    // Reads big file and learns based off it ... need to create new document...
    var numberOfReviews = 5261669;
    var i = 0;

    var reviewData = fs.openSync('./src/public/data/testdata.json', 'a');

    rl.on('line', function (line) {
        var reviewString = JSON.parse(line);
        //438160 5261670
        var numberofReviewsforTesting = 4194867;
        var reviewToSave = 85000;
...
var runTest = function runTest() {
    //Open big review file
    var instream = fs.createReadStream('./src/public/data/testdata.json');
    ...
    var positiveTest = 0;
    var negativeTest = 0;
    var numberOfline = 425000;
    //let numberOfline = 2000;

    var testResults = fs.openSync('./src/public/data/testResults3.json', 'a');
    ...
    rl.on('line', function (line) {
        if (!ended) {
            ...
            console.log(i);
            if (i < numberOfline) {
                var predictedClass = nbc.predict(JSON.stringify(reviewString.text));
...
var trainReviewData = function trainReviewData(reviewText, starRating) {
    nbc.importJSON(parsedData);
...
router.post('/test', function (req, res) {
    var predictedClass = nbc.predict(req.body.text);
...
//# sourceMappingURL=textClassification.js.map